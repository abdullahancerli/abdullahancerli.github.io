...
function addEnemy(){
    for(var i = enemies.length; i < maxenemy; i++){
        var r = Math.random() *30 +10; // Yuvarlak Balon Çapları
        var c = 'hsl('+(Math.random() * 360)+ ',40%,50%)'; // Balon Renkleri
        var s = .5 + ((40 - ((r/40) * r)) / 160) / maxenemy; // Hız Ayarları

        // Balonların Gelişleri Ayarlama
        var x,y;
        if(Math.random()< .5){
    ...
    enemies.forEach((enemy,e) => {
        bullets.forEach((bullet,b) => {
            if(collision(enemy.x,enemy.y,enemy.r,bullet.x,bullet.y,bullet.r)){
                // silah sesi
                var audio = new Audio('sound/bullet.wav');
            audio.play();
                if(enemy.r < 15){
                    enemies.splice(e,1);
                    score += 25;
        ...
        if (collision(enemy.x,enemy.y,enemy.r,player.x,player.y,player.r)){
            ...
            p.innerHTML ="OYUN BİTTİ ! <br> Puanın : " + score;
            playing = false;
            // oyun bitti sesi
...
function init(){
    ...
    var audio = new Audio('sound/ready2.wav');
            audio.play();
